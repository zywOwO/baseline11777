...
import sys
sys.path.append('/home/kkato/self-critical.pytorch')
...
def main(params):
  ...
  for i,img in enumerate(imgs):
    # load the image
    #if i< 1000:
    #  print(dir_att+'/'+str(img['filename'])+'.npy')
    if os.path.isfile(dir_fc+'/'+str(img['filename'])+'.npy') and os.path.isfile(dir_att+'/'+str(img['filename'])+'.npz'):
      print(dir_fc,str(img['filename']),'exist!')
    else:
      ...
      if max(I.shape) > 1920:
        I = skimage.transform.rescale(I, 1920/max(I.shape))
      ...
      if I.size(0) > 3:
        print('have alpha' )
        I = I[:3][:][:]
      print(params['images_root'], img['filepath'], img['filename'], I.size())
      ...
      np.save(os.path.join(dir_fc, str(img['filename'])), tmp_fc.data.cpu().float().numpy())
      np.savez_compressed(os.path.join(dir_att, str(img['filename'])), feat=tmp_att.data.cpu().float().numpy())
